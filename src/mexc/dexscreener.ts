...
function isBlockedBaseSymbol(symbol: string): boolean {
  ...
  const blockedExact = [
    "USD1", "STOCK", "NASDAQ", "NAS100", "SPX", "DJI",
    "TESLA", "NVIDIA", "APPLE", "MSFT", "SBUX", "ARM", "HD", "COPPER"
  ];
...
export class DexScreenerClient {
    ...
    for (let attempt = 1; attempt <= 4; attempt += 1) {
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          const retryAfterMs =
            Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
              ...
              : 0;
          const backoffMs = retryAfterMs > 0 ? retryAfterMs : attempt * 2500;

          logger.warn(
            { requestId, attempt, backoffMs, retryAfter, url, query },
        ...
        if (response.status === 400) {
          logger.warn(
            { requestId, attempt, url, query, durationMs },
        ...
        logger.warn(
          {
            ...
            error: errorMessage,
            isConnectionReset,
            isLikelyIPBlock: isConnectionReset && attempt === 1
          },
        ...
        logger.error(
          {
            ...
            query,
            isConnectionReset,
            isLikelyIPBlock: isConnectionReset
          },
          "DexScreener request retries exhausted - possible IP block"
        );
  ...
  private async findPairByQuery(query: string): Promise<DexPair | null> {
    ...
    const preferredChains = config.dexPreferredChains.map((item) =>
      item.trim().toLowerCase()
    );
    const quotePriority = config.dexQuotePriority.map((item) =>
      item.trim().toUpperCase()
    );

    const candidates = payload.pairs
      .filter((pair) => {
        const chainId = (pair.chainId ?? "").trim().toLowerCase();
        const quoteSymbol = (pair.quoteToken?.symbol ?? "")
          .trim()
          .toUpperCase();
        const baseSymbol = (pair.baseToken?.symbol ?? "").trim();
        ...
        if (!preferredChains.includes(chainId)) return false;
        if (!quotePriority.includes(quoteSymbol)) return false;
        if (isBlockedBaseSymbol(baseSymbol)) return false;
        if (normalizeSymbol(baseSymbol) !== normalizedQuery) return false;
        if (liquidityUsd < config.dexMinLiquidityUsd) return false;
        if (volumeM5 < config.dexMinVolumeM5Usd) return false;
        if (
          buysM5 < config.minDexBuysSellsM5 ||
          sellsM5 < config.minDexBuysSellsM5
        )
          return false;
        if (!(priceUsd > 0)) return false;
        if (pairCreatedAt > 0 && now - pairCreatedAt > maxPairAgeMs) return false;

        return true;
      })
      .map(
        (pair): DexPair => ({
          chainId: (pair.chainId ?? "").trim().toLowerCase(),
          dexId: pair.dexId ?? "",
      ...
      .sort((a, b) => {
        const quoteRankA = quotePriority.indexOf(
          a.quoteSymbol.trim().toUpperCase()
        );
        const quoteRankB = quotePriority.indexOf(
          b.quoteSymbol.trim().toUpperCase()
        );
        if (quoteRankA !== quoteRankB) return quoteRankA - quoteRankB;
        if (b.liquidityUsd !== a.liquidityUsd)
          return b.liquidityUsd - a.liquidityUsd;
        if (b.volumeM5 !== a.volumeM5) return b.volumeM5 - a.volumeM5;
  ...
  private getQueryAliases(query: string): string[] {
    ...
    if (base.startsWith("1000")) aliases.push(base.slice(4));
  ...
  async getPairByChainAndAddress(
    ...
    const url =
      `${this.baseUrl}/pairs/` +
      `${encodeURIComponent(chainId)}/${encodeURIComponent(pairAddress)}`;
    ...
    return {
      chainId: (pair.chainId ?? chainId).trim().toLowerCase(),
      dexId: pair.dexId ?? "",
  ...
  async getPairsByChainAndAddresses(
    ...
    for (
      ...
      offset += MAX_PAIRS_PER_BATCH_REQUEST
    ) {
      const chunk = uniqueAddresses.slice(
        offset,
        offset + MAX_PAIRS_PER_BATCH_REQUEST
      );
      ...
      const url =
        ...
        chunk.map((address) => encodeURIComponent(address)).join(",");
      ...
      if (!payload?.pairs?.length) {
        logger.warn(
          { chainId, requested: chunk.length, url },
      ...
      for (const rawPair of payload.pairs) {
        const priceUsd = Number(rawPair.priceUsd ?? 0);
        const pairAddress = rawPair.pairAddress ?? "";

        if (!pairAddress || !(priceUsd > 0)) continue;

        result.set(pairAddress.toLowerCase(), {
          chainId: (rawPair.chainId ?? chainId).trim().toLowerCase(),
          dexId: rawPair.dexId ?? "",
          pairAddress,
          baseTokenAddress: rawPair.baseToken?.address ?? "",
          quoteTokenAddress: rawPair.quoteToken?.address ?? "",
          baseSymbol: rawPair.baseToken?.symbol ?? "",
          quoteSymbol: rawPair.quoteToken?.symbol ?? "",
          liquidityUsd: Number(rawPair.liquidity?.usd ?? 0),
          volumeM5: Number(rawPair.volume?.m5 ?? 0),
          buysM5: Number(rawPair.txns?.m5?.buys ?? 0),
          sellsM5: Number(rawPair.txns?.m5?.sells ?? 0),
          priceUsd,
