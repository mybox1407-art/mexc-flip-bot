...
export const config = {
  ...
  // --- DEX trend filter ---
  dexTrendBlockPct: numberEnv(
    "DEX_TREND_BLOCK_PCT",
    0.5
  ),

  dexTrendWindowMs: numberEnv(
    "DEX_TREND_WINDOW_MS",
    60_000
  ),

  dexTrendMinPoints: numberEnv(
    "DEX_TREND_MIN_POINTS",
    5
  ),
  ...
  /**
   * Основной стоп после окончания
   * стартовой защитной фазы.
   */
  ...
  /**
   * Фаза 1: строгий защитный стоп
   * сразу после входа.
   */
  paperInitialStopLossPct: numberEnv(
    "PAPER_INITIAL_STOP_LOSS_PCT",
    0.4
  ),
   ...
   */
  paperInitialStopWindowMs: numberEnv(
    "PAPER_INITIAL_STOP_WINDOW_MS",
    20_000
  ),

  /**
   * Закрытие, если DEX-якорь движется
   * против открытой позиции относительно MEXC.
   */
  paperAnchorBreakPct: numberEnv(
    "PAPER_ANCHOR_BREAK_PCT",
    0.4
  ),
  ...
  paperMaxTotalExposurePct: numberEnv(
    "PAPER_MAX_TOTAL_EXPOSURE_PCT",
    0.6
  ),

  paperStopSlippagePct: numberEnv(
    "PAPER_STOP_SLIPPAGE_PCT",
    0.1
  ),

  paperMinHoldMs: numberEnv(
    "PAPER_MIN_HOLD_MS",
    30_000
  ),

  // --- Trailing stop ---
  paperTrailingTriggerPct: numberEnv(
    "PAPER_TRAILING_TRIGGER_PCT",
    0.5
  ),

  paperTrailingDistancePct: numberEnv(
    "PAPER_TRAILING_DISTANCE_PCT",
    0.3
  ),

  // --- Risk sizing ---
  paperRiskPerTradePct: numberEnv(
